...
const ProjectDetailsPage = () => {
  const { projectCode } = useParams(); // Using projectCode instead of projectId
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        const project = await projectsApi.getProject(projectCode);
        if (!project) {
        ...
        setError(err.response?.data?.message || "Unable to fetch project");
      } finally {
        setLoading(false);
      }
    };
    fetchProject();
  }, [projectCode]); // Dependency updated to projectCode
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="mt-6">
        ...
        >
          ...
          project.presentationAttendees.length > 0 ? (
